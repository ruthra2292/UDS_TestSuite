# run_all_tests_fixed.py
"""
Master test runner for UDS Test Suite - Windows Console Compatible
Executes all UDS service tests with proper encoding handling
"""
...
class UDSTestSuite:
    """Master UDS Test Suite Runner - Windows Compatible"""
    ...
    def generate_final_report(self, total_duration: float):
        """Generate comprehensive test report - Windows compatible"""
        print("\n" + "="*80)
        ...
        for name, result in self.results.items():
            ...
            print(f"{name:<35} {result['total']:<8} {result['passed']:<8} {result['failed']:<8} {result['pass_rate']:<7.1f}% {status_indicator} {result['status']:<8}")
        ...
        if overall_pass_rate == 100:
            print("SUCCESS: All tests passed! Your UDS implementation is compliant with ISO 14229.")
        elif overall_pass_rate >= 90:
            print("GOOD: Excellent compliance! Minor issues detected - review failed tests.")
        elif overall_pass_rate >= 75:
            print("WARNING: Good compliance with some issues - review and fix failed tests.")
        else:
            print("CRITICAL: Significant compliance issues detected - thorough review required.")
